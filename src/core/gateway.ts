...
import { parseAndRun } from "../policies";
import {
  JwtSessionStrategy,
...
export class Gateway {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    // 2) Policies before auth
    await parseAndRun(this.cfg.policies?.beforeAuth, {
      request,
      url,
      env: this.env,
      cfg: this.cfg,
    ...
    // 3) Route matching + session resolve
    const rule =
      new RouteMatcher(this.cfg.routes).match(url, request.method) ??
      { match: { path: "/**" }, auth: "required" };
    ...
    // 5) Policies after auth
    await parseAndRun(this.cfg.policies?.afterAuth, {
      request,
      url,
      env: this.env,
      cfg: this.cfg,
    ...
    // 6) Forward to FE or API via service bindings, with signed user + short-lived access token
  ...
  private pickProvider(explicit?: string) {
    ...
    return { ...adapter, config: p } as any;
  }
